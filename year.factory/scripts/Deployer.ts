	...
	public async deployProxy(contractParams: any[]) {
		try {
			this.contract = await upgrades.deployProxy(
				...
				[this.wallet.address, ...contractParams], // constructor arguments
				{ kind: 'uups' }
			)
			...
			this.contractAddress = this.contract.target.toString()
			console.log(`Deployed proxy contract ${this.contractName} at address ${this.contractAddress}`)
			return this.contract
		} catch (err: any) {
